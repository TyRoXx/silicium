...
BOOST_AUTO_TEST_CASE(asio_async_source)
{
	...
	Si::asio::async_source<decltype(background) &, decltype(source) &> async(background, source);
	std::array<int, 3> buffer;
	boost::asio::io_service foreground;
	bool ok = false;
	async.async_get(Si::make_contiguous_range(buffer), foreground,
	                [&ok, &buffer](Si::variant<Si::iterator_range<int const *>, int *> result)
	                {
		                BOOST_REQUIRE(!ok);
