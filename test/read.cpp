...
BOOST_AUTO_TEST_CASE(read_invalid)
{
	...
	Si::error_or<std::size_t> result = ventura::read(Si::no_file_handle, Si::make_memory_range(buffer));
...
BOOST_AUTO_TEST_CASE(read_write_pipe)
{
	...
	BOOST_CHECK_EQUAL(Si::error_or<std::size_t>(static_cast<std::size_t>(expected.size())), ventura::write(pipe.write.handle, expected));
	Si::error_or<std::size_t> result = ventura::read(pipe.read.handle, Si::make_memory_range(buffer));
