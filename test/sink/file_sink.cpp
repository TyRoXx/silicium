...
BOOST_AUTO_TEST_CASE(file_sink_append)
{
	...
	BOOST_REQUIRE_EQUAL(5u, Si::read(buffer.read.handle, Si::make_contiguous_range(read_buffer)).get());
	BOOST_CHECK_EQUAL_COLLECTIONS(expected.begin(), expected.end(), read_buffer.begin(),
