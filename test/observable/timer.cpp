...
BOOST_AUTO_TEST_CASE(asio_timer)
{
    ...
    auto coro = Si::make_total_consumer(Si::make_coroutine(
        [&t, &elapsed_count, loop_count](Si::yield_context yield)
        {
...
BOOST_AUTO_TEST_CASE(asio_owning_timer)
{
    ...
    t.async_get_one(
        Si::any_ptr_observer<std::shared_ptr<test_observer>>(observer));
