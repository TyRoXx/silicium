...
namespace bf
{
    ...
    Si::optional<command> detect_command(char c)
    {
        ...
        {
        case '>':
            return command::ptr_increment;
        case '<':
            return command::ptr_decrement;
        case '+':
            return command::value_increment;
        case '-':
            return command::value_decrement;
        case '.':
            return command::write;
        case ',':
            return command::read;
        case '[':
            return command::begin_loop;
        case ']':
            return command::end_loop;
        default:
            return Si::none;
        }
    }
    ...
    auto scan(Source &&source)
        ...
        -> Si::conditional_transformer<command,
                                       typename std::decay<Source>::type,
                                       Si::optional<command> (*)(char)>
#endif
    {
        return Si::transform_if_initialized(
            std::forward<Source>(source), detect_command);
    }

    template <class CommandRange>
    typename CommandRange::const_iterator
    find_loop_begin(CommandRange const &program,
                    std::unordered_map<std::ptrdiff_t,
                                       typename CommandRange::const_iterator>
                        &cached_loop_pairs,
                    typename CommandRange::const_iterator const end_)
    {
        using std::begin;
        auto cached =
            cached_loop_pairs.find(std::distance(program.begin(), end_));
        ...
        cached_loop_pairs.insert(
            std::make_pair(std::distance(program.begin(), end_), i));
        return i;
    }

    template <class CommandRange>
    typename CommandRange::const_iterator
    find_loop_end(CommandRange const &program,
                  std::unordered_map<std::ptrdiff_t,
                                     typename CommandRange::const_iterator>
                      &cached_loop_pairs,
                  typename CommandRange::const_iterator const begin)
    {
        using std::end;
        auto cached =
            cached_loop_pairs.find(std::distance(program.begin(), begin));
        if (cached != end(cached_loop_pairs))
        {
            return cached->second;
        }
        ...
        assert(current_depth == 0);
        cached_loop_pairs.insert(
            std::make_pair(std::distance(program.begin(), begin), i));
        return i;
    }

    template <class Input, class CommandRange, class MemoryRange>
    auto execute(Input &&input, CommandRange const &program,
                 MemoryRange &memory, std::size_t original_pointer)
#if !SILICIUM_COMPILER_HAS_AUTO_RETURN_TYPE
...
BOOST_AUTO_TEST_CASE(bf_empty)
{
    ...
    auto interpreter = bf::execute(
        input, Si::iterator_range<bf::command const *>(), memory, 0);
...
BOOST_AUTO_TEST_CASE(bf_hello_world)
{
    ...
    Si::empty<boost::uint8_t> input;
    auto interpreter = bf::execute(
        input, scan_all(Si::make_container_source(hello_world)), memory, 0);
    std::string printed;
    auto done =
        Si::for_each(std::move(interpreter), [&printed](boost::uint8_t output)
                     {
