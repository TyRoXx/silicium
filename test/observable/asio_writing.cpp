...
BOOST_AUTO_TEST_CASE(asio_writing_observable)
{
	...
	auto reading_observable = Si::asio::make_reading_observable(
	    reader, Si::make_memory_range(read_buffer));
	std::string received;
	reading_observable.async_get_one(Si::make_function_observer(
	    [&](boost::optional<Si::error_or<Si::memory_range>> element)
	    {
	...
	bool sent = false;
	writing_observable.async_get_one(Si::make_function_observer(
	    [&sent](boost::optional<boost::system::error_code> ec)
	    {
		    BOOST_REQUIRE(ec);
		    BOOST_REQUIRE(!sent);
		    BOOST_REQUIRE(!*ec);
