...
namespace Si
{
	...
	{
		auto co = Si::make_coroutine_generator<int>(
		    [](Si::push_context<int> &yield) -> void
		    {
			    yield(1);
			    yield(2);
			});
	...
	BOOST_AUTO_TEST_CASE(reactive_coroutine_get_one)
	{
		...
		auto co = Si::make_coroutine_generator<int>(
		    [&asyncs, &exited_cleanly](Si::push_context<int> &yield) -> void
		    {
		...
		// TODO: reading past the end should not be the required way to avoid a
		// force unwind of the coroutine
