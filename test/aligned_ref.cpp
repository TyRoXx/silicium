...
BOOST_AUTO_TEST_CASE(aligned_ref_ok)
{
    ...
    Si::optional<Si::aligned_ref<char, sizeof(buffer)>> a =
        Si::aligned_ref<char, sizeof(buffer)>::create(
...
BOOST_AUTO_TEST_CASE(aligned_ref_copy)
{
    ...
    Si::copy(*Si::aligned_ref<char const, sizeof(from)>::create(
                 reinterpret_cast<char const &>(from)),
             *Si::aligned_ref<char, sizeof(to)>::create(
