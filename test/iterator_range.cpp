...
BOOST_STATIC_ASSERT((std::is_same<Si::iterator_range<char const *>::value_type,
                                  char const>::value));
BOOST_STATIC_ASSERT(
    (std::is_same<Si::iterator_range<char *>::value_type, char>::value));
...
BOOST_AUTO_TEST_CASE(iterator_range_forward_iterators)
{
	...
	sstr.str("test");
	auto r = Si::make_iterator_range(
	    std::istream_iterator<char>(sstr), std::istream_iterator<char>());
