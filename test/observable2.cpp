...
{
	std::pair<Si::observables2::pipe_reader<int>,
	          Si::observables2::pipe_writer<int>> pipe =
	    Si::observables2::create_pipe<int>();
	bool first_callback = false;
	bool second_callback = false;
	auto transformed = Si::observables2::transform(
	    std::move(pipe.first), [&first_callback, &second_callback](int value)
	    {
