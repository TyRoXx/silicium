...
BOOST_AUTO_TEST_CASE(asio_use_observable_with_timer)
{
    ...
    Si::spawn_coroutine([&elapsed, &got_result](Si::spawn_context yield)
                        {
                            Si::optional<boost::system::error_code> result =
                                yield.get_one(elapsed);
...
BOOST_AUTO_TEST_CASE(asio_use_observable_with_socket)
{
    ...
    bool got_result = false;
    Si::spawn_coroutine(
        [&sent, &got_result, message](Si::spawn_context yield)
        {
            Si::optional<Si::error_or<std::size_t>> result =
                yield.get_one(sent);
            BOOST_REQUIRE(result);
            BOOST_REQUIRE(!got_result);
            BOOST_CHECK(result->is_error() ||
                        (result->get() == boost::asio::buffer_size(message)));
            got_result = true;
        });
    io.run();
    BOOST_CHECK(got_result);
