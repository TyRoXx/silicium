...
{
	Si::error_or<std::size_t> result = ventura::write(Si::no_file_handle, Si::make_c_str_range("test"));
...
BOOST_AUTO_TEST_CASE(write_empty)
{
	...
	Si::error_or<std::size_t> result = ventura::write(buffer.write.handle, Si::make_c_str_range(""));
...
BOOST_AUTO_TEST_CASE(write_non_empty)
{
	...
	Si::error_or<std::size_t> result = ventura::write(buffer.write.handle, message);
	BOOST_REQUIRE(!result.is_error());
	BOOST_CHECK_EQUAL(static_cast<size_t>(message.size()), result.get());
	std::array<char, 4096> read_buffer;
	result = ventura::read(buffer.read.handle, Si::make_contiguous_range(read_buffer));
	BOOST_REQUIRE(!result.is_error());
