...
{
	std::vector<char> expected = boost::assign::list_of(1)(2)(3);
	Si::iterator_range<char *> r = Si::make_memory_range(expected.data(), expected.data() + expected.size());
	BOOST_CHECK_EQUAL_COLLECTIONS(begin(expected), end(expected), r.begin(), r.end());
}
...
{
	std::vector<char> const expected = boost::assign::list_of(1)(2)(3);
	Si::iterator_range<char const *> r = Si::make_memory_range(expected.data(), expected.data() + expected.size());
	BOOST_CHECK_EQUAL_COLLECTIONS(begin(expected), end(expected), r.begin(), r.end());
}
