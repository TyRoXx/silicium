...
{
	silicium_observable *const coro = silicium_make_coroutine(
	    [](void *user_data, silicium_yield_context *yield) -> void *
	    {
...
{
	silicium_observable *const a = silicium_make_coroutine(
	    [](void *user_data, silicium_yield_context *yield) -> void *
	    {
		    boost::ignore_unused_variable_warning(user_data);
		    boost::ignore_unused_variable_warning(yield);
	    ...
	    nullptr);
	silicium_observable *const b = silicium_make_coroutine(
	    [](void *user_data, silicium_yield_context *yield) -> void *
	    {
