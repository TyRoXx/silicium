#include "silicium/html/tags.hpp"
#include <boost/test/unit_test.hpp>

using namespace Si::html;

BOOST_AUTO_TEST_CASE(empty_html)
{
...
BOOST_AUTO_TEST_CASE(tags_paragraph)
{
    std::string html_generated;
    auto erased_html_sink = Si::Sink<char, Si::success>::erase(
        Si::make_container_sink(html_generated));
    tags::p(tags::cl("someclass"), Si::html::text("Testing p tag"))
        .generate(erased_html_sink);
    BOOST_CHECK_EQUAL(
        "<p class=\"someclass\">Testing p tag</p>", html_generated);
}

BOOST_AUTO_TEST_CASE(tags_div)
{
    std::string html_generated;
    auto erased_html_sink = Si::Sink<char, Si::success>::erase(
        Si::make_container_sink(html_generated));
...
BOOST_AUTO_TEST_CASE(tags_span)
{
    std::string html_generated;
    auto erased_html_sink = Si::Sink<char, Si::success>::erase(
        Si::make_container_sink(html_generated));
    tags::span(tags::cl(""), Si::html::text("Testing span tag"))
        .generate(erased_html_sink);
    BOOST_CHECK_EQUAL(
        "<span class=\"\">Testing span tag</span>", html_generated);
}

BOOST_AUTO_TEST_CASE(tags_break_row)
{
    std::string html_generated;
    auto erased_html_sink = Si::Sink<char, Si::success>::erase(
        Si::make_container_sink(html_generated));
