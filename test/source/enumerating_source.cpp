...
{
	auto s = Si::make_enumerating_source(
	    Si::empty_source<Si::iterator_range<int *>>());
...
{
	auto s = Si::make_enumerating_source(
	    Si::make_single_source(Si::iterator_range<int *>()));
	Si::optional<int> e = Si::get(s);
	BOOST_CHECK(!e);
}
...
{
	int const element = 3;
	auto s = Si::make_enumerating_source(Si::make_single_source(
	    Si::iterator_range<int const *>(&element, &element + 1)));
	Si::optional<int> e = Si::get(s);
	BOOST_REQUIRE(e);
