...
BOOST_AUTO_TEST_CASE(zlib_stream_wrappers)
{
	...
	std::pair<std::size_t, std::size_t> decompress_result = inflator.inflate(
	    Si::make_iterator_range(
	        compressed.data(),
	        compressed.data() + compressed.size() - compress_result.second),
	    Si::make_iterator_range(
	        decompressed.data(), decompressed.data() + decompressed.size()),
	    Z_SYNC_FLUSH);
	auto const decompressed_length =
	    decompressed.size() - decompress_result.second;
	BOOST_REQUIRE_EQUAL(original.size(), decompressed_length);
	BOOST_CHECK_EQUAL(
	    original, std::string(decompressed.begin(),
	                          decompressed.begin() + decompressed_length));
}
...
BOOST_AUTO_TEST_CASE(zlib_deflating_sink_test)
{
	...
	auto compressor =
	    Si::make_deflating_sink(Si::make_container_buffer(compressed),
	                            Si::zlib_deflate_stream(Z_DEFAULT_COMPRESSION));
	Si::append(
	    compressor, Si::zlib_sink_element{Si::make_c_str_range("Hello")});
