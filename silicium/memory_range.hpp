...
namespace Si
{
	...
	struct copy_const
	{
		typedef typename boost::mpl::if_<boost::is_const<From>, boost::add_const<To>,
...
#endif
	{
		...
		return make_iterator_range(reinterpret_cast<destination *>(begin), reinterpret_cast<destination *>(end));
	}
...
#endif
	{
		...
		auto end_ = end(range);
		BOOST_STATIC_ASSERT((std::is_same<std::random_access_iterator_tag,
