...
#define SILICIUM_HAS_HTML_TREE (SILICIUM_COMPILER_HAS_VARIADIC_TEMPLATES && !SILICIUM_GCC46)
...
namespace Si
{
	namespace html
	{
...
#endif
			{
				return element<typename std::decay<ContentGenerator>::type, Length>{
		...
		template <std::size_t NameLength, class Attributes, class Element,
		          class ResultLength = typename detail::concatenate<
		              ...
		              typename detail::concatenate<
		                  ...
		                  exact_length<1 + (NameLength - 1) + 1 + 2 + (NameLength - 1) + 1>>::type>::type>
		...
		template <std::size_t NameLength, class Attributes,
		          class ResultLength = typename detail::concatenate<
		              min_length<1 + (NameLength - 1) + 2>, typename std::decay<Attributes>::type::length_type>::type>
...
#endif
		{
			return detail::make_element<ResultLength>(
			    ...
			    std::function<void(Sink<char, success>::interface &)>
#endif
			    ([&name, SILICIUM_CAPTURE_EXPRESSION(attributes, std::forward<Attributes>(attributes)) ](
			        Sink<char, success>::interface & destination)
			     {
				     html::open_attributed_element(destination, name);
				     attributes.generate(destination);
...
#if !SILICIUM_COMPILER_HAS_AUTO_RETURN_TYPE
		    -> decltype(tag(name, detail::make_element<exact_length<0>>(&detail::no_attributes),
		                    std::forward<Element>(content)))
#endif
		{
			return tag(name, detail::make_element<exact_length<0>>(&detail::no_attributes),
			           std::forward<Element>(content));
		}
		...
		template <std::size_t KeyLength, std::size_t ValueLength,
		          std::size_t ResultLength = detail::space + (KeyLength - 1) + detail::assign + detail::quote +
		...
		auto sequence(Head &&head, Tail &&... tail)
		    ...
		    -> detail::element<std::function<void(Sink<char, success>::interface &)>,
		                       typename detail::concatenate<typename std::decay<Head>::type::length_type,
...
#endif
		{
			...
			return detail::make_element<typename detail::concatenate<
			    typename std::decay<Head>::type::length_type, typename decltype(tail_elements)::length_type>::type>(
#if !SILICIUM_COMPILER_HAS_AUTO_RETURN_TYPE
			    std::function<void(Sink<char, success>::interface &)>
#endif
			    ([
