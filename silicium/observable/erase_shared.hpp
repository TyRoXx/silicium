...
namespace Si
{
    ...
    using shared_observable = ptr_observable<
        Element,
        std::shared_ptr<observable<Element, ptr_observer<observer<Element>>>>>;
#else
    template <class Element>
    struct shared_observable
        : ptr_observable<
              Element,
              std::shared_ptr<typename Observable<
                  Element, ptr_observer<observer<Element>>>::interface>>
    {
        typedef ptr_observable<
            Element, std::shared_ptr<typename Observable<
                         Element, ptr_observer<observer<Element>>>::interface>>
            base;
    ...
    auto erase_shared(Input &&input)
        -> Si::shared_observable<typename std::decay<Input>::type::element_type>
    {
        ...
        return Si::shared_observable<element_type>(
            std::make_shared<Si::virtualized_observable<
