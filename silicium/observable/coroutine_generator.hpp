...
#define SILICIUM_HAS_COROUTINE_GENERATOR                                       \
	((BOOST_VERSION >= 105300) && SILICIUM_HAS_EXCEPTIONS &&                   \
	 SILICIUM_HAS_VARIANT && SILICIUM_HAS_YIELD_CONTEXT &&                     \
...
namespace Si
{
...
#else
			typedef typename boost::coroutines::coroutine<
			    typename detail::make_command<Element>::type()>::caller_type
			    consumer_type;
#endif
	...
	struct coroutine_generator_observable : private observer<unit>
	{
		...
		coroutine_generator_observable &
		operator=(coroutine_generator_observable &&other)
		{
	...
	private:
		...
		void next()
		{
				...
				coro_ =
				    ...
				    ([bound_action](
				        ...
				        typename boost::coroutines::coroutine<
				            command_type>::push_type
#else
				        typename coroutine_type::caller_type
#endif
				            &push)
				     {
					     detail::coroutine_yield_context_impl<Element>
					         yield_impl(push);
					     push_context<Element> yield(
					         yield_impl); // TODO: save this indirection
			...
			if (coro())
			{
				...
				return Si::visit<void>(
				    ...
				    {
					    return Si::exchange(receiver_, nullptr)
					        ->got_element(std::move(*command.value));
					},
				    [this](detail::yield command)
				    {
					    command.target->async_get_one(observe_by_ref(
					        static_cast<observer<unit> &>(*this)));
					});
			}
			else
			{
		...
		SILICIUM_DELETED_FUNCTION(coroutine_generator_observable(
		    coroutine_generator_observable const &))
		SILICIUM_DELETED_FUNCTION(coroutine_generator_observable &operator=(
		    coroutine_generator_observable const &))
	};

	template <class Element, class Action>
	auto make_coroutine_generator(Action &&action)
	    -> coroutine_generator_observable<Element>
	{
		BOOST_STATIC_ASSERT_MSG(
		    (std::is_same<
		        void, decltype(action(
		                  std::declval<push_context<Element> &>()))>::value),
		    "The return type of the action has to be void because we do not "
		    "want "
		    "to accidentally "
		    "ignore a non-void result.");
		return coroutine_generator_observable<Element>(
