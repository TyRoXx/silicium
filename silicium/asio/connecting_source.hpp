...
namespace Si
{
    namespace asio
    {
        typedef std::shared_ptr<boost::asio::ip::tcp::socket>
            connecting_source_socket_ptr;
        typedef boost::variant<connecting_source_socket_ptr,
                               boost::system::error_code>
            connecting_source_element_type;

        struct connecting_source
        {
            typedef connecting_source_element_type element_type;

            explicit connecting_source(
                boost::asio::io_service &io, boost::asio::yield_context &yield,
        ...
        inline connecting_source::connecting_source(
            boost::asio::io_service &io, boost::asio::yield_context &yield,
            boost::asio::ip::tcp::endpoint remote_endpoint)
            : io(&io)
            , yield(&yield)
        ...
        inline iterator_range<connecting_source::element_type const *>
            connecting_source::map_next(std::size_t)
        {
            assert(io);
            assert(yield);
            return {};
        ...
        inline connecting_source::element_type *
        connecting_source::copy_next(iterator_range<element_type *> destination)
        {
            assert(io);
            assert(yield);
