...
namespace Si
{
	namespace asio
	{
		...
		struct async_source
		{
			typedef typename std::decay<Source>::type::element_type element_type;
			typedef Si::variant<iterator_range<element_type const *>, element_type *> result_type;
			...
			auto async_get(iterator_range<element_type *> buffer, HandlerDispatcher &dispatcher,
			               Handler &&handle_result)
			{
				return Si::asio::async(
				    ...
				    {
					    {
						    iterator_range<element_type const *> const mapped =
						        m_source.map_next((std::numeric_limits<std::size_t>::max)());
						    if (!mapped.empty())
						    {
							    return mapped;
						    }
				    ...
				    [SILICIUM_CAPTURE_EXPRESSION(handle_result, std::forward<Handler>(handle_result))](
