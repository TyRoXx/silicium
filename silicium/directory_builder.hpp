...
namespace Si
{
	struct directory_builder
	{
		...
		virtual std::unique_ptr<Si::flushable_sink<char, boost::system::error_code>> begin_artifact(std::string const &name) = 0;
	...
	struct filesystem_directory_builder SILICIUM_FINAL : directory_builder
	{
		...
		virtual std::unique_ptr<Si::flushable_sink<char, boost::system::error_code>> begin_artifact(std::string const &name) SILICIUM_OVERRIDE;
