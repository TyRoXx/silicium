...
namespace Si
{
	namespace http
	{
		...
		error_or<optional<http::request>> receive_request(AsyncStream &client, YieldContext &&yield)
		{
			std::array<char, 4096> incoming_buffer;
			auto receiver = asio::make_reading_observable(client, make_memory_range(incoming_buffer));
			auto without_errors = make_error_extracting_source(make_observable_source(std::move(receiver), yield));
			auto enumerated = make_enumerating_source(ref_source(without_errors));
