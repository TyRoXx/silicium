...
namespace Si
{
	namespace http
	{
		...
		void generate_status_line(CharSink &&out, Version const &version, Status const &status,
		...
		{
			generate_status_line(out, header.http_version, boost::lexical_cast<std::string>(header.status),
			                     header.status_text);
