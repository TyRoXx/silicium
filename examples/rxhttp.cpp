...
#define SILICIUM_EXAMPLE_AVAILABLE                                             \
    (SILICIUM_HAS_EXCEPTIONS && SILICIUM_HAS_YIELD_CONTEXT &&                  \
     SILICIUM_HAS_COROUTINE_GENERATOR)
...
namespace
{
    ...
    void serve_client(Socket &client, boost::uintmax_t visitor_number)
    {
        ...
        Si::optional<Si::http::request> request =
            Si::http::parse_request(bytes_receiver);
        if (!request)
        {
            return;
        }
        std::vector<char> send_buffer;
        {
            ...
            std::string const body = boost::str(
                boost::format("Hello, visitor %1%") % visitor_number);
            (*response.arguments)["Content-Length"] =
                boost::lexical_cast<Si::noexcept_string>(body.size());
            (*response.arguments)["Connection"] = "close";
            Si::http::generate_response(response_sink, response);
            Si::append(response_sink, body);
        }
        client.send(Si::make_iterator_range(
            send_buffer.data(), send_buffer.data() + send_buffer.size()));
        client.shutdown();
    ...
    typedef Si::asio::reading_observable<boost::asio::ip::tcp::socket>
        socket_observable;

    struct coroutine_socket
    {
        explicit coroutine_socket(boost::asio::ip::tcp::socket &socket,
                                  Si::push_context<Si::unit> &yield)
            : socket(&socket)
            , yield(&yield)
            , received(4096)
            , receiving_(Si::observable_source<socket_observable,
                                               Si::push_context<Si::unit>>(
                  socket_observable(
                      socket,
                      Si::make_iterator_range(
                          received.data(), received.data() + received.size())),
                  yield))
        {
        }
        ...
        void send(Si::iterator_range<char const *> data)
        {
            ...
            auto virtualized = Si::virtualize_observable<
                Si::ptr_observer<Si::observer<boost::system::error_code>>>(
                sending);
    ...
    private:
        ...
        Si::virtualized_source<Si::observable_source<
            socket_observable, Si::push_context<Si::unit>>> receiving_;
    };

    struct thread_socket_source
        : Si::Source<Si::error_or<Si::memory_range>>::interface
    {
        ...
        virtual Si::iterator_range<element_type const *>
        ...
        virtual element_type *copy_next(
            Si::iterator_range<element_type *> destination) SILICIUM_OVERRIDE
        {
            assert(socket);
            ...
            if (i != destination.end())
            {
                ...
                auto const bytes_received =
                    socket->receive(boost::asio::buffer(received), 0, ec);
                if (ec)
                {
                    *i = ec;
                }
                ...
                {
                    *i = Si::make_memory_range<char const>(
                        received.data(), received.data() + bytes_received);
                }
                ++i;
            }
            return i;
    ...
    struct thread_socket
    {
        ...
        void shutdown()
        {
            assert(receiving_.get_socket());
            boost::system::error_code error;
            receiving_.get_socket()->shutdown(
                boost::asio::ip::tcp::socket::shutdown_both, error);
        }
    ...
    {
        explicit coroutine_web_server(boost::asio::io_service &io,
                                      boost::uint16_t port)
            : acceptor(io, boost::asio::ip::tcp::endpoint(
                               boost::asio::ip::address_v4(), port))
            , clients(&acceptor)
            , all_work(Si::make_total_consumer(Si::erase_unique(
                  Si::flatten<
                      boost::mutex>(Si::make_coroutine_generator<events>(
                      [this](Si::push_context<events> &yield) -> void
                      {
                          ...
                          for (;;)
                          {
                              ...
                              auto context = [visitor_count, client]() -> events
                              {
                                  ...
                                  auto client_handler = Si::erase_shared(
                                      Si::make_coroutine_generator<Si::unit>(
                                          [client, visitor_number_](
                                              Si::push_context<Si::unit> &yield)
                                              -> void
                                          {
                                              coroutine_socket coro_socket(
                                                  *client, yield);
                                              return serve_client(
                                                  coro_socket, visitor_number_);
                                          }));
    ...
    {
        explicit thread_web_server(boost::asio::io_service &io,
                                   boost::uint16_t port)
            : acceptor(io, boost::asio::ip::tcp::endpoint(
                               boost::asio::ip::address_v4(), port))
            , clients(&acceptor)
            , all_work(Si::make_total_consumer(
                  Si::erase_unique(Si::flatten<boost::mutex>(
                      Si::make_coroutine_generator<events>(
                          [this](Si::push_context<events> &yield) -> void
                          {
                              boost::uintmax_t visitor_count = 0;
                              for (;;)
...
int main()
{
    ...
    std::for_each(workers.begin(), workers.end(),
                  [](boost::unique_future<void> &worker)
                  {
                      worker.get();
                  });
#else
